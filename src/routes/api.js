...
router.get("/nsummary", async (req, res) => {
  ...
  const resources = resourceKeys
    .map((key) => key.split("|"))
    .map((arr) => ({ type: arr[0], attribute: arr[1], datatype: arr[2] }));
  ...
  for (let res of resources) {
    ...
    const data = await axios
      .get(
        `/exec?cmd=mean&resourceType=${type}&resourceAttribute=${attribute}&sites=${req.query.sites}`,
        passAuth(req),
      )
      .then((res) => res.data);
    ...
    if (res.datatype === "number") {
      ...
      const mean = conns
        .map((conn) => conn.value.mean * (conn.value.n / totalN))
        .reduce(sum);
  ...
  var response = {
    ...
    figures: [
      ...
      },
      {
        ...
        data: [
          [20, 18, 16, 14, 10, 10, 8, 9, 6, 5],
          [23, 21, 19, 17, 15, 13, 10, 9, 8, 6],
        ],
      },
    ],
  };
  ...
  fs.readFile("./test/summary.json", "utf8", (err, jsonString) => {
    ...
    let categories = Object.keys(raw),
      ...
      summaryData = _.map(raw, (v, k) => {
        return _.map(raw[k].data, (va, ke) => {
          if (sites.indexOf(ke) > -1) return [va, ke];
        });
      });

    summaryData = _.map(summaryData, (cat) => {
      return _.filter(cat, (e) => {
        return e !== undefined;
      });
    });

    let site = _.map(summaryData[0], (e) => {
      return e[1];
    });
    ...
    if (request.variables) {
      if (request.variables.indexOf("length_of_stay") >= 0) {
        ...
        let length_of_stay = _.map(summaryData[idx], (elm) => {
          return [[elm[0], raw.length_of_stay.range[elm[1]], elm[1]]];
        });
      ...
      if (request.variables.indexOf("age_groups") >= 0) {
        let header = ["age"].concat(
            _.filter(
              Object.keys(age_group[Object.keys(age_group)[0]]),
              (site) => {
