...
function queryParamString(req: Request, key: string): string | undefined {
  // @ts-ignore
  const obj: string | ParsedQs | string[] | ParsedQs[] | undefined =
    req.query[key];
...
router.get("/query_task_1", async function (req: Request, res: Response) {
  try {
    res
      .status(200)
      .send(
        JSON5.parse(getCachedFile(req.app, "./mock/tasks/ret_task_1.json5")),
      );
  } catch (err) {
    console.error(err.stack);
    res.status(500).send("Unable to fetch site info from Hub");
...
router.get("/query_task_2", async function (req: Request, res: Response) {
  try {
    res
      .status(200)
      .send(
        JSON5.parse(getCachedFile(req.app, "./mock/tasks/ret_task_2.json5")),
      );
  } catch (err) {
    console.error(err.stack);
    res.status(500).send("Unable to fetch site info from Hub");
