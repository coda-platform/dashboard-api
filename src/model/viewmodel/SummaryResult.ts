...
function dataFromBreakdown(req: SummarizeRequestBody, obj: any) {
  ...
  if (obj[0][0].breakdown.fieldType == "dateTime") {
    breakResult = sortBy(
      (obj[0][0].breakdown.result ?? []).map((r: any) => ({
        periodStart: r.periodStart.slice(0, 10),
        periodCount: r.periodCount,
      })),
      "periodStart",
    ); //slice dateTime to remove time and only keep the date
  } else {
    breakResult = sortBy(
      (obj[0][0].breakdown.result ?? []).map((r: any) => ({
        periodStart: parseFloat(r.periodStart),
        periodCount: r.periodCount,
      })),
      "periodStart",
    );
  ...
  return [
    {
      about: {
        field: obj[0][0].breakdown.field,
        fieldType: obj[0][0].breakdown.fieldType,
      },
      cols: [
        ...
        { code: "total", labels: { fr: "total", en: "total" } },
        {
          ...
          categories: (breakResult ?? []).map((r: any) => ({
            code: r.periodStart,
          })),
        },
      ],
          ...
          tmp = {
            ...
            cols: [
              ...
              { code: "mode", labels: customLabels("mode") },
              {
                code: "count",
                labels: customLabels("count"),
                categories: (result.count ?? []).map((c: { label: any }) => ({
                  code: c.label,
                })),
              },
            ],
...
function customLabels(code: string): I18nString {
  switch (code) {
    case "site":
      return { fr: "site", en: "site" };
    case "total":
      return { fr: "total", en: "total" };
    case "mode":
      return { fr: "mode", en: "mode" };
    case "count":
      return { fr: "décompte", en: "count" };
    default:
...
export default class SummaryResult {
  static fromSummary(
    ...
    return {
      request: request,
      data: requestHasBreakdown(request)
        ? dataFromBreakdown(request, obj)
        : dataFromFields(request, obj),
    };
  }
}

function fieldsFromSelector(
