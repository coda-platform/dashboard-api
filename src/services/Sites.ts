...
import {
  ...
  SummarizeRequestBreakdownSliceResult,
  SummarizeRequestResult,
  SummarizeRequestSiteAllResult,
  SummarizeRequestSiteResult,
...
function getSites2Total(res: SummarizeRequestResult): Map<string, number> {
  return new Map(
    res.map((s: SummarizeRequestSiteAllResult) => [
      code2name(s[0].siteCode),
      s[0].total,
    ]),
  );
}

export type PeriodBreakdownPerSite = {
  dates: Date[];
  sites: { [site: string]: number[] };
};

function getPeriodBreakdownPerSite(
  res: SummarizeRequestResult,
): PeriodBreakdownPerSite {
  // Each site has an array of results with only the first filled for reasons unknown to me
  const firstResults: SummarizeRequestSiteResult[] = res.map(
    (arr: SummarizeRequestSiteResult[]) => arr[0],
  );
  ...
  const dates = sitesDf[0].breakdown.result.map(
    (rec: SummarizeRequestBreakdownSliceResult) => new Date(rec.periodStart),
  );

  // Load site data
  const sites: { [site: string]: number[] } = {};
  for (let site of sitesDf) {
    sites[code2name(site.siteCode)] = site.breakdown.result.map(
      (rec: SummarizeRequestBreakdownSliceResult) => rec.periodCount,
    );
  }
...
function getAllTotal(sites2total: Map<string, number>): Promise<number> {
  ...
  else return Promise.reject("No Aggregated all value");
}
...
export class Sites {
  ...
  listConnected(): Promise<SiteInfoRequest> {
    ...
    return axios
      .get(`/info?waitTime=${infoWaitTime}`, passAuth(this.req))
  ...
  getActiveIcuOnDate(
    ...
    const icuCountPerSitesBetweenDates = SRBuilder.newSel("Encounter")
      ...
      .join(SRBuilder.newSel("Location").filterIs("type.coding.code", "ICU"));
    ...
    // Get summary results.
    return this.doSummaryQuery(sr, sitesCode).then(getSites2Total);
  }
  ...
  getNewDailyPosPerSiteBetween(from: Date, to: Date, sitesCodes?: string[]) {
    ...
    // Get summary results.
    return this.doSummaryQuery(sr, sitesCodes).then(getPeriodBreakdownPerSite);
  }
  ...
  getNewDailyIcuPerSiteBetween(from: Date, to: Date, sitesCodes?: string[]) {
    const icuCountPerSitesBetweenDates = SRBuilder.newSel(
      "Encounter",
  ...
  getNewDailyWardPerSiteBetween(from: Date, to: Date, sitesCodes?: string[]) {
    const wardCountPerSitesBetweenDates = SRBuilder.newSel(
      "Encounter",
  ...
  doSummaryQuery(
    ...
    sitesCodes?: string[],
  ): Promise<SummarizeRequestResult> {
    ...
    // Get summary results.
    return axios
      .get(`/stats/summarize?sites=${sitesCodes.join(",")}`, reqCfg)
      .then(getSRData);
  }
  ...
  prepare(request: any, sitesCodes?: string[]) {
    ...
    try {
      return axios
        .get(`/learning/prepare?sites=${sitesCodes.join(",")}`, data)
        .then((res: any) => res.data);
    } catch (err) {
      return err;
    }
  ...
  train(request: any, sitesCodes?: string[]) {
    ...
    try {
      return axios
        .get(`/learning/train?sites=${sitesCodes.join(",")}`, data)
        .then((res: any) => res.data);
    } catch (err) {
      return err;
    }
  ...
  progress(request: any, sitesCodes?: string[]) {
    ...
    try {
      return axios
        .get(`/learning/progress?sites=${sitesCodes.join(",")}`, data)
        .then((res: any) => res.data);
    } catch (err) {
      return err;
    }
  ...
  evaluate(request: any, sitesCodes?: string[]) {
    ...
    try {
      return axios
        .get(`/learning/evaluate?sites=${sitesCodes.join(",")}`, data)
        .then((res: any) => res.data);
    } catch (err) {
      return err;
    }
